...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
    ...
    if (user != null && mounted) {
      setState(() {
        ...
        _salarioController.text =
            user.salarioMensal.toStringAsFixed(2).replaceAll('.', ',');
        _necessidades = user.percentualNecessidades;
        _objetivos = user.percentualObjetivos;
        _reserva = user.percentualReserva;
      });
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      final existing = await ref.read(getUserUseCaseProvider).call();
      final user = UserEntity(
        id: existing?.id,
        nome: _nomeController.text.trim(),
        salarioMensal: _salarioController.text.parseBrl,
        percentualNecessidades: _necessidades,
        percentualObjetivos: _objetivos,
        percentualReserva: _reserva,
        dataCriacao: existing?.dataCriacao ?? DateTime.now(),
      );
      await ref.read(saveUserUseCaseProvider).call(user);
      ref.invalidate(currentUserProvider);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text(AppStrings.dadosSalvos)),
        );
        context.pop();
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text('Perfil', style: AppTextStyles.label),
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: AppStrings.rendaMensal,
                prefixText: 'R\$ ',
              ),
            ),
            const SizedBox(height: 32),
            Text('Distribuição da Renda', style: AppTextStyles.label),
            const SizedBox(height: 16),
  ...
  Widget _buildDistributionSection() {
    final salario = _salarioController.text.parseBrl;
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            _LegendItem(AppColors.necessidades,
                '🏠 ${_necessidades.toInt()}%'),
